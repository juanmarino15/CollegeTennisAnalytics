        ...
        <div className="space-y-4">
          {doublesMatches.map((match) => (
            <div key={match.id} 
              ...
              <div className="font-semibold text-gray-900 dark:text-dark-text mb-2 flex justify-between items-center">
              #{match.position}
...
<div className={`text-sm mt-1 flex justify-between items-center
  ${match.side1_won ? 'text-green-600 dark:text-green-400 font-semibold' : 'text-gray-600 dark:text-gray-400'}`}>
  <div>
    {formatPlayerName(players[match.side1_player1_id])} / {formatPlayerName(players[match.side1_player2_id])} [{teams.home.abbreviation}]
    {match.side1_won && <Check className="w-4 h-4 inline ml-1" />}
  </div>
  <div className="ml-8">
...
{/* Away Team Players */}
<div className={`text-sm mt-1 flex justify-between items-center
  ${match.side2_won ? 'text-green-600 dark:text-green-400 font-semibold' : 'text-gray-600 dark:text-gray-400'}`}>
  <div>
    {formatPlayerName(players[match.side2_player1_id])} / {formatPlayerName(players[match.side2_player2_id])} [{teams.away.abbreviation}]
    {match.side2_won && <Check className="w-4 h-4 inline ml-1" />}
  </div>
  <div className="ml-8">
    {match.side1_score.split(' ')[0].split('-')[1].split('(')[0]}
  </div>
</div>
                             </div>
            </div>
...
<div className={`text-sm mt-1 flex justify-between items-center
  ${match.side1_won ? 'text-green-600 dark:text-green-400 font-semibold' : 'text-gray-600 dark:text-gray-400'}`}>
  <div>
    {formatPlayerName(players[match.side1_player1_id])} [{teams.home.abbreviation}]
    {match.side1_won && <Check className="w-4 h-4 inline ml-1" />}
  </div>
...
<div className={`text-sm mt-1 flex justify-between items-center
  ${match.side2_won ? 'text-green-600 dark:text-green-400 font-semibold' : 'text-gray-600 dark:text-gray-400'}`}>
  <div>
    {formatPlayerName(players[match.side2_player1_id])} [{teams.away.abbreviation}]
    {match.side2_won && <Check className="w-4 h-4 inline ml-1" />}
  </div>
  <div className="ml-8 space-x-4">
    {match.side1_score.split(' ').map((set, index) => (
      <span key={index}>{set.split('-')[1].split('(')[0]}</span>
    ))}
  </div>
</div>
